...
PROGRAM MONOLITHICSCHEMEADVECTIONDIFFUSIONFIELDMLEXAMPLE
  ...
  INTEGER(CMISSIntg) :: IndependentFieldUserNumberDiffusion
  ...
  TYPE(CMISSFieldType), ALLOCATABLE, DIMENSION(:) :: IndependentFieldDiffusion
...
!   CALL CMISSDiagnosticsSetOn(CMISSInDiagType,DIAG_LEVEL_LIST,"Diagnostics",DIAG_ROUTINE_LIST,Err)
  ...
  ALLOCATE (IndependentFieldDiffusion(Ncompartments))
...
! !  ENDIF
  ...
  DO icompartment = 1,Ncompartments
    ...
    CALL CMISSEquationsSetCreateStart(EquationsSetUserNumberDiffusion,Region,GeometricField,&
         ...
         & CMISSEquationsSetAdvectionDiffusionEquationType,CMISSEquationsSetMultiCompTransportAdvecDiffSubtype,&
...
!     & MaterialsFieldUserNumberMatPropertiesPermOverVis,PERM_OVER_VIS_PARAM_MAT_PROPERTIES,Err)
  ...
  !SOURCE FIELDS
   ...
   !create the equations set source field variables for all equations sets 
  DO icompartment = 1,Ncompartments
  ...
  DO icompartment = 1,Ncompartments
    ...
    CALL CMISSEquationsSetIndependentCreateStart(EquationsSetDiffusion(icompartment),IndependentFieldUserNumberDiffusion,&
         & IndependentFieldDiffusion(icompartment),Err)
    CALL CMISSEquationsSetIndependentCreateFinish(EquationsSetDiffusion(icompartment),Err)
  END DO 
...
!         CONDITION=CMISSBoundaryConditionFixed
!           VALUE=0.0_CMISSDP
!           CALL CMISSBoundaryConditionsSetNode(BoundaryConditionsDiffusion,DependentField,CMISSFieldU1VariableType,CMISSNoGlobalDerivative, & 
!             & NODE_NUMBER,MESH_COMPONENT_NUMBER_CONC_ONE,CONDITION,VALUE,Err)
!       ENDDO
!     ENDIF
!    ENDIF
...
!         CONDITION=CMISSBoundaryConditionFixed
!           VALUE=0.0_CMISSDP
!           CALL CMISSBoundaryConditionsSetNode(BoundaryConditionsDiffusion,DependentField,CMISSFieldU2VariableType,CMISSNoGlobalDerivative, & 
!             & NODE_NUMBER,MESH_COMPONENT_NUMBER_CONC_ONE,CONDITION,VALUE,Err)
!       ENDDO
!     ENDIF
!    ENDIF
...
!         CONDITION=CMISSBoundaryConditionFixed
!           VALUE=1.0_CMISSDP
!           CALL CMISSBoundaryConditionsSetNode(BoundaryConditionsDiffusion,DependentField,CMISSFieldU3VariableType,CMISSNoGlobalDerivative, & 
!             & NODE_NUMBER,MESH_COMPONENT_NUMBER_CONC_ONE,CONDITION,VALUE,Err)
!       ENDDO
!     ENDIF
!    ENDIF
...
END PROGRAM MONOLITHICSCHEMEADVECTIONDIFFUSIONFIELDMLEXAMPLE
